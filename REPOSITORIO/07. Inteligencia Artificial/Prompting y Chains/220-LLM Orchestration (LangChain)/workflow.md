---
title: Master Workflow: LLM Orchestration (LangChain)
version: 1.1
...
subcategory: General
tags: ['langchain', 'langgraph', 'llamaindex', 'chains', 'agents', 'llm-orchestration', 'rag', 'multi-agent-systems', 'prompt-engineering']
...
# 🚀 Estrategia de Automatización: LLM Orchestration (LangChain)

Este documento define las tres vías posibles para implementar la skill de LLM Orchestration (LangChain) en cualquier aplicación del ecosistema Antigravity.
...
```json
{
  ...
  "skill_target": "LLM Orchestration (LangChain)",
...
def execute_skill_automation(input_params):
    """Lógica operativa automatizada de LLM Orchestration (LangChain)"""
    print(f"Iniciando flujo técnico para: LLM Orchestration (LangChain)")
    # TODO: Implementar lógica basada en SKILL.md
    return {"status": "success", "skill": "LLM Orchestration (LangChain)"}
...
**REST Endpoint:** `POST /v1/automate/220`
